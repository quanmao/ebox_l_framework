...
void eboxInit(void)
{
    ...
    systick_clock_source_config(SYSTICK_CLOCK_SOURCE_AHBCLK_NODIV); //systemticks clock；		
...
void eboxGetClockInfo(Clock_t *clock)
{
	// 不从系统获取，直接赋值的方式可以减少大约800byte的code
		crm_clocks_freq_type RCC_Clock;

		system_core_clock_update();
		crm_clocks_freq_get(&RCC_Clock);

    clock->core = RCC_Clock.sclk_freq;
    clock->hclk = RCC_Clock.ahb_freq;
    clock->pclk2 = RCC_Clock.apb2_freq;
    clock->pclk1 = RCC_Clock.apb1_freq;
}
...
void eboxGetChipInfo(CpuInfo_t *cpu)
{
	...
	#define FLASHSIZE_BASE_ADDRESS	0x1FFFF7E0
 ...
 *@retval   NONE
*/
void eboxRccCmd(const DevToRcc_t *devtable, uint32_t dev, confirm_state state)
{
//	crm_periph_clock_enable(CRM_GPIOA_PERIPH_CLOCK,TRUE);
		...
		while(devtable[i].dev != 0)
		{
		  ...
		  if(TRUE == state)
  {
    CRM_REG(devtable[i].rcc) |= CRM_REG_BIT(devtable[i].rcc);
  }
