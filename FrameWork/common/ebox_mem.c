...
static uint32_t heap_addr_begin, heap_addr_end; // 内存的起始结束地址
...
void eboxHeapInit(void *begin_addr, void *end_addr)
{
...
void *eboxMalloc( size_t xWantedSize )
{
    ...
    {
        eboxHeapInit((void *)MCU_HEAP_BEGIN, (void *)MCU_HEAP_END);
    }
...
void *eboxRealloc(void *ptr, size_t size)
{
    if(ptr == NULL)
    {
         ptr = eboxMalloc(size);//distribution new mem
        return ptr;
    }
    else
    {
        void *temp = ptr;//save ptr to temp
        if(size < eboxGetSizeofPtr(ptr))
            return ptr;
        ptr = eboxMalloc(size);//distribution new mem
        if(ptr != NULL)
        {
            eboxMemCpy(ptr, temp, eboxGetSizeofPtr(temp));
            eboxFree(temp);//free temp
        }
        else
            return NULL;
    }
...
//因为需要有一个结构体描述当前内存块，所以实际大小最是比申请的大小大8
size_t eboxGetSizeofPtr(void *ptr)
{
    ...
    if((temp->blockSize & BlockAllocatedBit) != 0) //如果最高位为1，说明是有效的内存单元
        ...
        return (temp->blockSize & (~BlockAllocatedBit));
    else
        return 0;
}

size_t eboxGetFree(void)
{
...
{
    return (100 - eboxGetFree() * 100.0 / mem_size_aligned);
}
...
{
    return (mem_size_aligned - eboxGetFree());
    // 返回已用内存大小 
    //return ((heap_addr_end - heap_addr_begin) - (eboxGetFree()+2 * SIZEOF_STRUCT_MEM));
}
...
uint16_t eboxGetFreeBlock(void)
{
    eboxBlockLink_t *p;
    int i = 0;
//    MEM_DEBUG("\r\n------------------start------------------\r\n");
//    MEM_DEBUG(" free block\taddres\t|size\t|\r\n");
    for(p = (eboxBlockLink_t *)( &(heap[0]) ); p != NULL; p = ( p->nextFreeBlock))
    {
			i++;
//        MEM_DEBUG("free block %d: ", i++);
//        MEM_DEBUG("|@0X%X\t|%06d\t|\r\n", p, p->blockSize);
    }
//    MEM_DEBUG("-------------------end-------------------\r\n");
    return i - 2;
 ...
 */
int eboxMemCmp(const void *cs, const void *ct, size_t count)
{
 ...
 *
 */
void *eboxMemCpy(void *dst, const void *src, size_t count)
{
