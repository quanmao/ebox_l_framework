...
void E_EXTI::begin(PinMode_t mode)
{
    ...
    LL_GPIO_AF_SetEXTISource(GETEXTIPORT(_pin->id), EXTISorce_MAP[GETPINNUMBER(_pin->id)]);
...
#endif
#endif
    IRQn_Type index = eboxDevToIrq(IrqTableEXTI, (uint32_t)_extiLine, 0);
    eboxNvicIrqSetPriority((IRQn_Type)index, priority);
