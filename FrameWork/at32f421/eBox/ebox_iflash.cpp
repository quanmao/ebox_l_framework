...
E_FlashStates ebox_getFlashStates(void)
{
    ...
    if (FLASH_GET_FLAG(FLASH_WRPERR))			return FLASH_ERROR_WRP;
...
int E_FLASH::write(uint32_t offsetAdd, uint8_t *buf, uint32_t iNbrToWrite)
{
    ...
    while ( 1 )
    {
        ...
        else	//不够写入，移到下一个扇区
        {
            ...
            if (secpos >= _end_addr)
            {
                IFLASH_DEBUG("write: check flash size\r\n");
                return -1;
            }
            secoff = 0;//偏移位置为0
            buf += secremain;  //指针偏移
...
#if _DEBUG		
		int i = ebox_flashRead(iAddress, buf, iNbrToRead);
		IFLASH_DEBUG("read size %d\r\n",i);
		return i;
#else
