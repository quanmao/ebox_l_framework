...
void E_SPI::config(C_SpiConfig_t *spi_config)
{
    ...
    if (spi_config->dataWidth < 4 && spi_config->dataWidth > 16) _dataWidth = 8;
    LL_SPI_Disable(_spi);

    switch (spi_config->mode)
    {
    case C_SpiModeLowRise0:
        LL_SPI_SetClockPhase(_spi, LL_SPI_POLARITY_HIGH);
    ...
    LL_SPI_SetDataWidth(_spi, d_with[_dataWidth-4]);
    LL_SPI_SetRxFIFOThreshold(_spi, (_dataWidth <= 8) ? LL_SPI_RX_FIFO_TH_QUARTER : LL_SPI_RX_FIFO_TH_HALF);
...
DATA_T E_SPI::transfer(DATA_T data)
{
    if (_dataWidth <= 8)
    {
        ...
        *((__IO uint8_t *)&_spi->DR) = data;
        ...
        return (uint8_t)(READ_REG(_spi->DR));
    }
    else
    {
        while ((_spi->SR & LL_SPI_SR_TXE) == RESET)
            ;
        *((__IO uint16_t *)&_spi->DR) = data;
        while ((_spi->SR & LL_SPI_SR_RXNE) == RESET)
            ;
        return (uint16_t)_spi->DR;
    }
}
/**
  *@brief    写数据
