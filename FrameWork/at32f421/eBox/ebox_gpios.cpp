...
extern void port_mode(gpio_type* port,uint32_t pin, PinMode_t mode,uint8_t af_config=0);
...
void E_PORT::write(uint16_t val)
{
  // _port->ODR = (_port->ODR & ~_mask) | ((val << _offset) & _mask);
	 _port->odt = (_port->odt & ~_mask) | ((val << _offset) & _mask);
}
...
uint16_t E_PORT::read(void)
{
 //  return (_port->IDR & _mask) >> _offset;
	   return (_port->idt & _mask) >> _offset;
}
